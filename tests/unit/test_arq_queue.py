...
class _CollidingArqPool(_FakeArqPool):
    """Simulates arq returning None because `_job_id` collides with a still-queued job."""
...
async def test_enqueue_returns_caller_job_id_on_collision() -> None:
    """arq returns None when `_job_id` collides with an already-queued job — the idempotency
    key did its job, so this must not raise."""
    queue = ArqJobQueue(_CollidingArqPool())
    job_id = await queue.enqueue("ingest_document", _envelope(), job_id="idem-key-2")
    assert job_id == "idem-key-2"
